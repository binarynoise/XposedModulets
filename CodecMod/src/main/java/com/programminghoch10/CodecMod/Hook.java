...
public class Hook implements IXposedHookLoadPackage {
    ...
    MediaCodecInfo[] getFilteredMediaCodecInfos() throws InvocationTargetException, IllegalAccessException {
        ...
        final int codecCount =
            (int) XposedBridge.invokeOriginalMethod(XposedHelpers.findMethodExact(MediaCodecList.class, "getCodecCount"), null, null);
        for (int i = 0; i < codecCount; i++)
            mediaCodecs.add((MediaCodecInfo) XposedBridge.invokeOriginalMethod(
                XposedHelpers.findMethodExact(MediaCodecList.class, "getCodecInfoAt"),
                null,
                new Object[]{ i }
            ));
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam lpparam) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            XposedHelpers.findAndHookMethod(
                MediaCodecList.class, "getCodecInfos", new XC_MethodReplacement() {
                    @Override
                    protected Object replaceHookedMethod(MethodHookParam param) throws Throwable {
                        MediaCodecInfo[] mediaCodecInfos =
                            (MediaCodecInfo[]) XposedBridge.invokeOriginalMethod(param.method, param.thisObject, param.args);
            ...
            XposedHelpers.findAndHookMethod(
                MediaCodecList.class, "getCodecCount", new XC_MethodReplacement() {
                    @Override
            ...
            XposedHelpers.findAndHookMethod(
                MediaCodecList.class, "getCodecInfoAt", int.class, new XC_MethodReplacement() {
                    @Override
                    protected Object replaceHookedMethod(MethodHookParam param) {
